...
Before I begin, I'll very quickly recap the functional options pattern
----
...
----
We start with some options, expressed as functions which take a pointer to a `Config` structure, we pass those functions to some constructure type function, in this case called `NewTerrain`, and inside the body of that function each option function is invoked in order, passing in a reference to its `Config` value.
...
Subtraction, multiplication, and exponentiation are similarly easy to implement.
----
...
And because this is a server, all of these methods may be called concurrently so we need to use a Mutex to protect the `conns` map and prevent data races.
...
Now locking is no longer needed because `conns` is never shared. 
Instead other goroutines interact with `loop` via various channels.

But there are a few problems still to fix. The most pressing is the error handling in `SendMsg` is missing; an error writing to a connection will not be communicated back to the caller.
