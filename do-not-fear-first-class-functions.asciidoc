...
Roughly two years ago I stood on a stage, not unlike this one, and told you my opinion for how configuration options should be handled in Go.
...
Over the last two years it has been wonderful to watch this idea mature from Rob's original blog post, to my presentation, to the gRPC project, who have continued to evolve this design pattern into what I consider to be its best form to date.

But, when I was talking to Gophers at a conference in London a few months ago, several of them expressed a concernt that while _they_ understood the notion of a function that _returns_ a function, the technique that powers function options, they were worried that other Go programmers--I suspect insinuating possibly less experienced programmers--would not be able to understand this style of programming.
...
We pass those functions to some constructor type function, in this case called `NewTerrain`, and inside the body of that function each option function is invoked in order, passing in a reference to its `Config` value.

Ok, everyone should be familar with this pattern.

Where I believe the concern comes from is when you need an option function that takes parameters.
----
...
Finally, putting it all together, we call `NewTerrian` with the results of evaluating `WithReticulatedSplines` and `WithCities`.
...
Hopefully this pattern is familar to you.
