...
----
Hopefully you also noticed that the signature of our `Sqrt` function is the same as `math.Sqrt`, so we can make this code smaller by _resuing_ any function from the `math` package.
----
...
Suppose we're building a chat server, the next Hipchat or Slack, but we'll start small for the moment.
----
include::do-not-fear-first-class-functions/mutex.go[]
----
Here's the first cut of the heart of any chat system.
We have a way to register new connections, remove old ones, and send a message to all the open connections.
...
fcf's aren't just shorter to use, and more expressive, they are more reusable because in general, they do not have a name
...
They aren't hard, just a little unfamiliar, and unfamiliarity is something that can be overcome
