...
The assembly was for the tricky partsA
...
The Redzone is a 128 byte  allocation at the bottom of the stack frame which is guarneteed to be available.
...
Inlining also ameleorates the cost of the stack check preamble, and in fact all the overheads of a function call, by copying the code of the inlined function into it's caller.
...
Note: as of Go 1.8, you cannot combine ++//go:nosplit++ and ++//go:systemstack++
...
From the point of view of code executing on a single processor code, all reads and writes to main memory happen instantaniously.
...
However, from the point of view of another core connected to that 
...
They are used inside the runtime itself to gain a foothold to implement the runtime, including the garbage collector, in Go itself.
